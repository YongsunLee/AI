...
bool WifesGlobalState::OnMessage(MinersWife* wife, const Telegram& msg)
{
	...
	case Msg_MomImHome:
		...
		cout << "\n" << GetNameOfEntity(wife->ID()) <<
			": Oh My Baby. Let me make you some of French fries";

		wife->GetFSM()->ChangeState(CookFrenchFries::Instance());
...
//--------------------------------------------
CookFrenchFries * CookFrenchFries::Instance()
{
	static CookFrenchFries instance;

	return &instance;
}

void CookFrenchFries::Enter(MinersWife * wife)
{
	if (!wife->Cooking())
	{
		cout << "\n" << GetNameOfEntity(wife->ID()) << ": Temperature Not Ready";
...
void CookFrenchFries::Execute(MinersWife * wife)
{
	cout << "\n" << GetNameOfEntity(wife->ID()) << ": Fussin' over food";
}

void CookFrenchFries::Exit(MinersWife * wife)
{
	SetTextColor(FOREGROUND_GREEN | FOREGROUND_INTENSITY);

	cout << "\n" << GetNameOfEntity(wife->ID()) << ": Puttin' the FrenchFries on the table";
}

bool CookFrenchFries::OnMessage(MinersWife * wife, const Telegram & msg)
{
	SetTextColor(BACKGROUND_RED | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	...
	case Msg_FrenchFriesReady:
	{
		...
		SetTextColor(FOREGROUND_GREEN | FOREGROUND_INTENSITY);
		cout << "\n" << GetNameOfEntity(wife->ID()) << ": FrenchFriesReady! Lets eat";
