...
{       
   ...
   if (On(wall_avoidance))
  {
    force = WallAvoidance(m_pVehicle->World()->Walls()) *
            m_dWeightWallAvoidance;// 벽피하기를 얼마나 우선순위에 둘 것인지 가중치를 곱한다.

	// 벽 피하는 힘을 누적한다.
	// 실패하면 밑의 기능들이 실행이 되지 않는다. 
	// 성공 :: 여력이 남으면 다른 힘을 더 더하러 작동
...
{        
  ...
  // 모든 행동을 다 더해서
  // 최대 힘보다 넘을 가능성이 높기 때문에
  // Truncate를 통해서 제어
  // 방향(노말라이즈)만 구해서 Max 값으로 이용
...
{  
  ...
  //reset the steering force
   ...
   // 벽 피하기가 켜져 있고
   // 랜덤 float 가 prWallAvoidance보다 작을 때만 구한다?
  if (On(wall_avoidance) && RandFloat() < Prm.prWallAvoidance)
  {
    m_vSteeringForce = WallAvoidance(m_pVehicle->World()->Walls()) *
                         m_dWeightWallAvoidance / Prm.prWallAvoidance;	// prWallAvoidance 작게 하면 발생이 더 자주된다.
