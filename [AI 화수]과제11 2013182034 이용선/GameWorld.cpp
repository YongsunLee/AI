            ...
            m_bShowCellSpaceInfo(false)
{
  ...
  m_index = new int[Prm.NumAgents];

  for (int i = 0; i < Prm.NumAgents; ++i) {
	  if (i == 0)	m_index[i] = Prm.NumAgents-1;
	  else			m_index[i] = -1;
  }
...
#define SHOAL
  ...
  m_Vehicles[Prm.NumAgents-1]->Steering()->ArriveOn();
  m_Vehicles[Prm.NumAgents-1]->SetMaxSpeed(60.0f);


   for (int i=0; i<Prm.NumAgents-1; ++i)
  {
    m_Vehicles[i]->Steering()->WanderOn();
	   //if (i == 0) m_Vehicles[i]->Steering()->WanderOn(m_Vehicles[Prm.NumAgents - 1], Vector2D{5,5});
	   //else m_Vehicles[i]->Steering()->OffsetPursuitOn(m_Vehicles[i-1], Vector2D{ 5,5 });
  }
#endif
...
{ 
  ...
  for (unsigned int a = 0; a<m_Vehicles.size(); ++a)
  {
	  if ((m_Vehicles[m_Vehicles.size() - 1]->Pos() - m_Vehicles[a]->Pos()).Length() < 30.0f) {
		  for (int i = 0; i < m_Vehicles.size(); ++i) {

			  if (m_index[i] == a) break;

			  if (m_index[i] == -1) {
				  m_Vehicles[a]->Steering()->WanderOff();
				  m_Vehicles[a]->Steering()->PursuitOn(m_Vehicles[m_index[i - 1]]);
				  m_index[i] = a;
				  break;
			  }
		  }
