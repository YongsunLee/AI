...
void Raven_SensoryMemory::SetOpponentDamaged(Raven_Bot* pOpponent)
{
	MemoryMap::iterator it = m_MemoryMap.find(pOpponent);

	if (it != m_MemoryMap.end())
	{
		it->second.bDamaged = true;
	}
}

bool Raven_SensoryMemory::GetOpponentDamaged(Raven_Bot* pOpponent)const
{
	MemoryMap::const_iterator it = m_MemoryMap.find(pOpponent);

	if (it != m_MemoryMap.end())
	{
		return it->second.bDamaged;
	}
